...
class LocalDataStore extends ChangeNotifier {
  ...
  LocalDataStore._internal() {
    _initDummyData(); // Re-enabled to restore mock data
  }

  /// Loads items from Hive local storage. Call this after Hive is initialized.
  ...
  // Optionally, expose a stream for real-time updates from Firebase
  // Stream<List<ItemModel>> get itemsStream => _firebaseItemService.getAllItems();
  ...
  /// Adds a single new item and creates a history entry. Persists to Hive.
  ...
  /// Updates an existing item and creates a history entry. Persists to Hive.
  ...
  void checkoutItem(String itemId, String assignee) {
    ...
    /// Deletes an item from inventory and Hive.
    void deleteItem(String itemId) {
      ...
      if (index != -1) {
        ...
        _history.add(HistoryEntry(
          title: "Item Deleted",
          description: "Item ${removedItem.name} was deleted.",
          timestamp: DateTime.now(),
